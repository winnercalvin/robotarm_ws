# tacobot/drizzle_tools_spline_interpolation.py
import math
import time
...
def catmull_rom_spline(points, num_points=5):
    """
    Catmull-Rom spline 보간
    points: [{'x':..,'y':..}, ...]
    num_points: 구간당 생성할 보간 점 개수
    """
    if len(points) < 4:
        return points
...
def custom_drizzle(flat_data):
    ...
    if not filtered_path:
        print("   >>> ❌ 그릴 좌표가 없습니다.", flush=True)
        return

    # --------------------------
    # 3️⃣ Spline 보간 추가 🔥
    # --------------------------
    spline_path = catmull_rom_spline(filtered_path, num_points=5)

    print(f"   >>> 원본 점 개수: {len(filtered_path)}")
    print(f"   >>> 보간 후 점 개수: {len(spline_path)}")

    # --------------------------
    # 현재 위치 가져오기
    # --------------------------
    response = get_current_posx(ref=DR_BASE)
    current_pos = response[0] if isinstance(response, tuple) else response
    bx, by, bz, ba, bb, bc = current_pos

    # --------------------------
    # 경로 따라 이동
    # --------------------------
    for i, pt in enumerate(spline_path):

        tx = bx + (pt['x'] * DRAWING_SIZE)
        ty = by - (pt['y'] * DRAWING_SIZE)

        target = posx([tx, ty, bz - Z_PRESS_DEPTH, ba, bb, bc])

        if i == 0:
            movel(posx([tx, ty, bz, ba, bb, bc]),
                  vel=VELOCITY, acc=ACC, ref=DR_BASE)

            movel(target,
                  vel=VELOCITY/2, acc=ACC,
                  ref=DR_BASE)

            wait(0.2)

        elif i < len(spline_path) - 1:
            movel(target,
                  vel=VELOCITY,
                  acc=ACC,
                  radius=RADIUS,
